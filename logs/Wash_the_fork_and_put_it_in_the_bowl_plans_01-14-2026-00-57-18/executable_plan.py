...
ground_truth = [{'name': 'Bowl', 'contains': ['Fork'], 'state': None}]
no_trans_gt = 1
max_trans = 1
total_exec = 0
success_exec = 0
...
def PutObject(robot, put_obj, recp):
    print(f"Putting: {put_obj} into {recp}")
    robot_name = robot['name']
    agent_id = int(robot_name[-1]) - 1
    ...
    if recp_obj_id is None:
        print(f"ERROR: Receptacle '{recp}' not found! Available objects: {[o for o in objs if recp in o][:5]}")
        return

    print(f"Found receptacle: {recp_obj_id}")
    global recp_id
...
exec = 0.0 if total_exec == 0 else float(success_exec) / float(total_exec) # revise
