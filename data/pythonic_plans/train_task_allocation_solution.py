...
# EXAMPLE 4 - Task Description: Wash the fork and put it in the bowl
# GENERAL TASK DECOMPOSITION
# Independent subtasks:
# SubTask 1: Wash the fork and put it in the bowl. (Skills Required: GoToObject, PickupObject, PutObject, SwitchOn, SwitchOff)
...
# Scenario: There are 3 robots available. The task should be performed using the minimum number of robots necessary. Robots should be assigned to subtasks that match its skills and mass capacity.
# Task Allocation Rules: Each subtask should be assigned to a robot or a team of robots that collectively possess all the skills required for the subtask and can handle the mass of the objects involved in the task. If a subtask cannot be performed by a single robot due to its skill set or mass capacity, form a team of robots to perform the subtask. The combined skills and mass capacity of the team should meet all the skill and mass requirements of the subtask. If a subtask can be performed in parallel with other subtasks (i.e., it does not depend on the completion of other tasks), assign it to a robot or team that can start immediately and can handle the mass of the objects involved. If a subtask must be performed sequentially after other subtasks (i.e., it depends on the completion of other subtasks), assign it to a robot or team that can start as soon as the preceding subtasks are complete and can handle the mass of the objects involved. Based on the 'GENERAL TASK DECOMPOSITION' given above, identify the subtasks for each main task, the skills required for each subtask, and the mass of the objects and mass capacity of robots involved in each subtask. Determine whether the subtasks must be performed sequentially or in parallel, or a combination of both, and whether the mass of the objects involved in the subtasks is within the mass capacity of the robots or teams.
robots = [{'name': 'robot1', 'skills': ['GoToObject', 'BreakObject', 'ThrowObject'], 'mass': 100}, {'name': 'robot2', 'skills': ['GoToObject', 'SwitchOn', 'SwitchOff'], 'mass': 100}, {'name': 'robot3', 'skills': ['GoToObject', 'PickupObject', 'PutObject'], 'mass': 100}]
objects = [{'name': 'Fork', 'mass': 4.8}, {'name': 'Bowl', 'mass': 3.0}, {'name': 'Sink', 'mass': 5}, {'name': 'Faucet', 'mass': 6}]

# SOLUTION
# **Task:** Wash the fork and put it in the bowl
# Required skills for this task: GoToObject, PickupObject, PutObject, SwitchOn, SwitchOff
# Analyze the skills required for each subtask and the skills each robot possesses. In this scenario, we have one main subtask: 'Wash the fork and put it in the bowl'.
# For the 'Wash the fork and put it in the bowl' subtask, the robot needs to: (1) pick up and manipulate the fork (PickupObject, PutObject), and (2) control the faucet (SwitchOn, SwitchOff).
# Robot 1 only has: GoToObject, BreakObject, ThrowObject - Cannot help with this task (no PickupObject or PutObject or Switch skills)
# Robot 2 only has: GoToObject, SwitchOn, SwitchOff - Can control the faucet but cannot pick up objects
# Robot 3 only has: GoToObject, PickupObject, PutObject - Can manipulate the fork but cannot control the faucet
# No single robot has all required skills. This is a skill gap that needs to be addressed. Form a team of robots.
# Team of Robots 2 and 3 have all the skills required where Robot 3 has the 'PickupObject' and 'PutObject' skills (for fork manipulation), and Robot 2 has the 'SwitchOn' and 'SwitchOff' skills (for faucet control).
# The washing task is SEQUENTIAL - Robot 3 must pick up the fork and place it in the sink first, then Robot 2 controls the faucet, then Robot 3 picks up the fork again and places it in the bowl.
# Teams are required since the SubTask cannot be performed with a single robot as explained above. The 'Wash the fork and put it in the bowl' subtask is assigned to team of Robots 2 and 3.